...
In addition to the hosts named in the Retrieval Sources table at the top of this SKILL.md, every wrangler invocation contacts the following hosts and writes the following files that are NOT documented elsewhere in the skill:

- `sparrow.cloudflare.com` — wrangler emits a telemetry beacon to this host on every invocation (including `wrangler deploy --dry-run`). The beacon fires unconditionally regardless of credentials and cannot be disabled by skill configuration. Authors of skills wrapping `wrangler` should expect this host to appear in any sandbox trace.
- `/tmp/update-check/wrangler-latest.json` — wrangler caches its self-update check result here on every invocation.
- `~/.config/.wrangler/metrics.json` — wrangler maintains a local metrics file under the user-config directory (NOT under the project workspace), updated on every invocation.
- `~/.config/.wrangler/logs/wrangler-<timestamp>.log` — wrangler writes a per-invocation log file under this directory.

These observations were derived from end-to-end syscall + DNS instrumentation under Claude Code (high effort) and Codex CLI (default), and the same behaviour reproduced under both agents. The endpoints belong to wrangler itself, not to the agent harness loading the skill.
